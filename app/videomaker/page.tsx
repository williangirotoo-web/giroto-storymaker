  ...
  return (
    <div style={{ fontFamily: 'system-ui,sans-serif', background: '#0a0806', color: '#f0f0f0', minHeight: '100vh', padding: '40px 24px' }}>
      <div style={{ maxWidth: 1100, margin: '0 auto' }}>
        ...
        <h1 style={{ fontSize: '2rem', fontWeight: 800, color: '#fff', margin: '20px 0 8px' }}>Videomaker em Umuarama PR</h1>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(280px,1fr))', gap: 12 }}>
          {paginasVideo.map(p => (
            <a key={p.slug} href={'/videomaker/' + p.slug} style={{ background: 'rgba(212,168,67,0.05)', border: '1px solid rgba(212,168,67,0.15)', borderRadius: 10, padding: '14px 16px', textDecoration: 'none', color: '#ccc', fontSize: 13, display: 'block' }}>
              🎬 {p.title}
            </a>
          ))}
        </div>
