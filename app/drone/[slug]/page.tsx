...
import { getPaginaDroneBySlug, paginasDrone } from '@/data/paginas_drone'
...
export async function generateStaticParams() {
  return paginasDrone.map(p => ({ slug: p.slug }))
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const p = getPaginaDroneBySlug(params.slug)
  if (!p) return {}
  ...
  return {
    ...
    alternates: { canonical: `${SITE}/drone/${p.slug}` },
...
export default function PaginaDrone({ params }: Props) {
  const pagina = getPaginaDroneBySlug(params.slug)
  if (!pagina) notFound()
      ...
      <div style={{ fontFamily: 'system-ui,sans-serif', background: '#0a0806', color: '#f0f0f0', minHeight: '100vh' }}>
        ...
        <section style={{ background: 'linear-gradient(135deg,#1a0808 0%,#0a0806 70%)', padding: '60px 24px 40px', textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
          ...
          <div style={{ maxWidth: 800, margin: '0 auto', position: 'relative', zIndex: 1 }}>
            <div style={{ display: 'inline-block', background: 'rgba(212,68,67,0.15)', border: '1px solid rgba(212,68,67,0.4)', borderRadius: 20, padding: '6px 16px', marginBottom: 20 }}>
              <span style={{ color: '#e05050', fontSize: 12, fontWeight: 600, letterSpacing: 2 }}>🎬 DRONE EM UMUARAMA PR</span>
            </div>
            <h1 style={{ fontSize: 'clamp(1.6rem,5vw,3rem)', fontWeight: 800, color: '#fff', lineHeight: 1.2, marginBottom: 20 }}>{pagina.title}</h1>
            <p style={{ color: '#aaa', fontSize: 16, lineHeight: 1.7, maxWidth: 580, margin: '0 auto 12px' }}>Cícero Manoel Domingos — Piloto de Drone profissional em Umuarama PR. Filmagem, edição e entrega de vídeos para casamentos e eventos.</p>
        ...
        <section style={{ background: '#111', padding: '50px 24px', borderTop: '1px solid rgba(212,168,67,0.1)', textAlign: 'center' }}>
          <div style={{ maxWidth: 600, margin: '0 auto' }}>
            ...
            <p style={{ color: '#999', marginBottom: 8 }}>Studio Foto Danielle · Drone Umuarama PR</p>
        ...
        <footer style={{ background: '#050403', borderTop: '1px solid rgba(212,168,67,0.15)', padding: '24px', textAlign: 'center' }}>
          <p style={{ color: '#444', fontSize: 12 }}>© 2025 Studio Foto Danielle · Cícero Manoel Domingos · Drone Umuarama PR</p>
